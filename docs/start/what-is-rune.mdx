---
title: What is Rune?
lede: Rune is a lightweight, single-binary orchestration platform. Here's what it does, what it doesn't, and when to reach for it.
---
...
- **Kernel-level packet filtering for network policy.** Ingress and service-to-service egress rules *are* enforced, in the service proxy — see [network policies](/guides/network-policy). What is not enforced is traffic that never reaches the proxy: outbound connections to the internet or to raw container IPs.
