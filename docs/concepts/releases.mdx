...
as a verifiable back-pointer. GC and drift detection become deterministic.
...
dependency order (services → configmaps/secrets → volumes), and **volume reclaim
policy is honored**: a `retain` volume is released, not destroyed.
...
Because `Owns` is authoritative, `rune release status X` compares the recorded
desired state against live state and reports resources deleted out of band,
modified, or unhealthy — impossible without a persisted release.
...
There is no refcounting (a deliberate choice — it's a bug farm). Orphaned shared
resources can be reclaimed later with an explicit prune, never via runtime counters.
...
release can be reinstalled or rolled back. `--purge` removes the record; server-side
GC compacts old tombstones so they don't grow unbounded.
