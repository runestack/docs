---
title: Services
lede: A service is the unit of deployment in Rune — a versioned, reconciled, scaled workload.
---
...
A service can declare services, secrets, and configmaps it depends on. The reconciler waits until each dependency is ready before creating instances:
...
A service dependency is ready when at least one of its instances is running — and, if it declares a readiness probe, that probe is passing. Secrets and configmaps just need to exist. See [Service dependencies](/guides/dependencies) for the full semantics, and inspect with [`rune deps`](/cli/deps).
