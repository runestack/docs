...
### `Stalled` — stuck in create

When create fails repeatedly against a stable precondition — an unpullable image, a rejected registry credential, a missing secret or storage class — the reconciler exhausts its retries and freezes the record in `Stalled`. It stops auto-retrying on purpose: a service that cannot pull its image would otherwise burn instance IDs forever. The slot stays held by that record, so no replacement appears either.

A `Stalled` instance never had a container, so there is nothing to inspect with `rune logs` or `rune exec`. Read the cause off the service:

```sh
rune describe service api -n default
```

Fix the cause, then re-arm the slot:

```sh
rune restart api
```

Since v0.0.1-dev.141 `rune restart` clears the create-attempt counter and the retry backoff for these records, so the reconciler tries again immediately. See [Recovering instances stuck in create](/cli/restart-stop#recovering-instances-stuck-in-create).
