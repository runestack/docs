...
│       │                    │                  │                  │
│       ▼                    ▼                  ▼                  │
│   <svc>.<ns>.rune    container IP        policy evaluator        │
│       │                    │                  │                  │
│       └────────►  data plane proxy  ◀─────────┘                  │
│                                                                  │
...
4. **Network policy** — `ServiceNetworkPolicy` objects compile to per-VIP rule sets that the agent evaluates **in the data plane proxy**, for both ingress and egress. There is no kernel-level dropping today: the `nftables` programmer is a no-op on every platform ([#194](https://github.com/runestack/rune/issues/194)), so a connection that never reaches the proxy — to the internet, or straight to a container IP — is never filtered.
...
- No change to policy enforcement: it evaluates in the proxy in every mode, dev or production.
...
- **Service-name selectors are same-node only in v1.** Cross-node identity (so `from: { service: web }` matches web instances on *other* nodes) lands with Phase 2 alongside Raft. CIDR selectors work everywhere today.
- **Egress is narrower than ingress.** Because enforcement lives in the proxy, egress rules only constrain connections addressed to another service's VIP, and only when the agent can identify the source — which excludes `runtime: process` services and cross-node sources, where the connection is allowed rather than denied. See the [guide's limitations](/guides/network-policy#v1-limitations-to-know-about) before treating it as a security boundary.
