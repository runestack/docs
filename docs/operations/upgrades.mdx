---
title: Upgrades
lede: How to upgrade runed and the rune CLI safely. Rollback, version skew, what pauses during the swap, and the Terraform pitfalls.
---
...
`rune version` prints both client and server build info — use it to spot skew at a glance.
...
The recommended in-place upgrade path is `scripts/upgrade-server.sh`. Run it as root on the host:
...
- `--refresh-unit` — replace the on-disk `runed.service` with a fresh one from `runed print-systemd`. See [Refreshing the systemd unit](#refreshing-the-systemd-unit) below.
...
The `runed` binary itself emits its canonical unit via `runed print-systemd`, and `upgrade-server.sh --refresh-unit` uses that to swap in a current unit during the upgrade.
...
The module renders `var.rune_version` into the droplet's `user_data` (cloud-init). Cloud-init runs **only on first boot** — bumping `rune_version` in code does not re-run the installer on an existing droplet.

Pin the module to **≥ 0.0.6**, which sets `lifecycle { ignore_changes = [user_data] }` on the droplet so the variable can advance freely in code. Without that, Terraform's default behaviour on a `user_data` change is to **mark the droplet for replacement** (destroy + create), wiping `/var/lib/rune` (KEK, BadgerDB store, host-local volumes).
