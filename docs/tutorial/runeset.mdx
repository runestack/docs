...
lede: Write the multi-service runeset that deploys your app — one reusable cast template, per-environment values, plus configmaps and secrets.
---
...
`namespace` is only the default — `runeset.yaml` itself is not templated. Pass `-n dev` / `-n prod` at cast time so each environment lands in its own namespace from the same files.
...
Once created, secrets are encrypted at rest in `runed`'s BadgerDB. There is no read-API — the only way to consume them is via `envFrom: [{ secret: web-env }]` in your cast. Updating a secret requires a service restart:
