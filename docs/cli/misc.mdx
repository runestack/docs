...
If `Status: Not connected to server`, run `rune login` again or check that the server is reachable. The `Server Version` line is omitted if the server doesn't expose `HealthService.GetServerVersion`.
...
:::note
A bare `rune status` is the **whole-cluster** view. `-A` /
`--all-namespaces` selects the same thing explicitly; use `-n <ns>` to
...
The server probe uses the unauthenticated `HealthService.GetServerVersion` RPC so it works before `rune login`. If the server doesn't expose it, or you have no configured context, you'll see a one-line note instead of the `Server:` block; use `--client` to skip the probe entirely.
