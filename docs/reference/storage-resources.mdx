...
**Provider auth failures surface on the instance.** When a driver
reports that the provider rejected its credentials, the reason is
attached to the instance that is blocked waiting for the volume, rather
than only appearing in the agent log. Read it with `rune describe
...
than something you infer by counting warnings.
