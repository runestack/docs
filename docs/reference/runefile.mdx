...
Since v0.0.1-dev.139 the `registry` field is matched against the
image's **repository path**, not just its host:

| `registry` pattern              | Matches                                                            |
| ------------------------------- | ------------------------------------------------------------------ |
| `ghcr.io`                       | Every image on the host — the historical behaviour.                 |
| `ghcr.io/myorg`                 | Only `ghcr.io/myorg` and repositories beneath it.                   |
| `*.dkr.ecr.us-east-1.amazonaws.com` | Host wildcard, as before.                                      |
...
then the longer pattern. Precedence no longer depends on declaration
order. An image no entry claims gets no configured credential and pulls
anonymously.
