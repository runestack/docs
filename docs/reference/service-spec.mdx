...
There is nothing else to configure. How many spare copies an update may run, how far below `scale` it may dip, how long a replacement must hold before it counts as serving, and when a stuck update gives up are all **derived per service** rather than set by hand.
