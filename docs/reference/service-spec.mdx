...
| `imageRegistry`| string  | no       | Named registry selector. Schema accepted; not honoured by the runner yet. See [`registry` / `imageRegistry`](#registry-imageregistry). |
| `registry`     | object  | no       | Per-service registry override. Schema accepted; not honoured by the runner yet. See [`registry` / `imageRegistry`](#registry-imageregistry). |
...
touching the runefile. See [Registry credential scoping](/guides/ghcr-auth#scope-a-credential-to-a-repository-path).

[Init steps](#initsteps) and the `--debug` sidecar inherit the flag —
they run the same image.

Available since v0.0.1-dev.139. `cast` rejected the field in dev.139
itself; upgrade to v0.0.1-dev.140 or later to use it.

### `registry` / `imageRegistry`

```yaml
registry:
  name: ghcr-private     # names a configured docker.registries[] entry
  auth:
    type: basic          # basic | token | ecr
    username: ${USER}
    password: ${PAT}

imageRegistry: ghcr-private
```

Both fields parse and validate (`basic` requires `username` + `password`,
`token` requires `token`), but **the Docker runner does not read them
yet** — credential selection is entirely server-side, from
`docker.registries[]`. Configure registry credentials in the
[runefile](/reference/runefile#dockerregistries) or with
[`rune admin registry`](/cli/admin), and use
[`imagePullAnonymous`](#imagepullanonymous) for the per-service opt-out
that *is* wired up.
