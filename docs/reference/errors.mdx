...
Since v0.0.1-dev.139 an authentication failure is annotated with which
registry pattern was involved — see the two entries below.

### `no [[docker.registries]] entry matches <image>`

The registry demanded credentials and no configured entry claimed the
image, so the pull went out anonymously. Usually a `registry` pattern
scoped past the repository. Widen it, or add an entry. See
[`docker.registries[]`](/reference/runefile#dockerregistries).

### `the credential configured for registry pattern "X" was rejected`

An entry matched and the registry refused its credential. Either renew
it, or — if the image is **public** and only inherited the credential
from a broader pattern — scope the pattern to your private repositories,
add a `type: none` entry for the public path, or set
