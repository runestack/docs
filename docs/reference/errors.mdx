...
- An entry matched but declares `type: none`, so it deliberately sends
  nothing. The image is not public after all; give it a real credential.
- The service sets `imagePullAnonymous: true`.

Check the last two before widening a pattern — in both, no pattern was
consulted, so editing one changes nothing. See
...
The bare form (with `(will retry)`) is transient. When the driver has a
concrete failure, it is appended — e.g. `storage driver: provider
rejected credentials: … -> HTTP 401 (check the storage class's
apiToken)`, which is terminal and needs the token renewed. See
