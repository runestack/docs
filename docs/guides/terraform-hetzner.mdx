...
lede: Use the official terraform-hetzner-rune module to spin up a Rune edge node on a Hetzner Cloud server — server, firewall, cloud-init install, and an optional one-shot admin bootstrap.
---
