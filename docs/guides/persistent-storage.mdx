...
The driver probes for that device link **before** calling the DO API.
The link only exists for volumes attached to *this* droplet, so its
presence proves attachment — and the volume name is derived locally.
The common restart therefore makes **zero** provider API calls, and
does not depend on the `apiToken` still being valid.
...
When the driver has a concrete failure, the message carries it, so you
don't have to go log-diving:
