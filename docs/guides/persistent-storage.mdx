...
:::note
The reference is re-resolved on every driver operation, detach
included, so a rotation takes effect without remounting the volume or
restarting `runed`.
:::
...
The `do-volume` driver probes for that device link **before** calling the
DO API. The link only exists for volumes attached to *this* droplet, so
its presence proves attachment — and the volume name is derived locally.
The common restart therefore makes **zero** provider API calls, and does
not depend on the `apiToken` still being valid.

This shortcut is specific to `do-volume`. The other cloud drivers detect
an existing attachment through the provider API, so a restart on those
still needs a working credential.
...
When the agent can report a concrete failure, the message carries it in
place of `(will retry)`, so you don't have to go log-diving. The cause is
the driver's own error, wrappers and all:

```text
volume prod/pgdata-postgres-0 not yet mounted on node node-abc:
agent.volumes: attach prod/pgdata-postgres-0: dovolume: getVolume
<handle>: storage driver: provider rejected credentials: GET
/v2/volumes/<id> -> HTTP 401: {"id":"unauthorized","message":"Unable to
authenticate you"} (check the storage class's apiToken)
```
...
Read the cause, not the absence of `(will retry)` — that wording is
dropped whenever the agent has *any* cause to report, transient or not.

A credential rejection (HTTP 401/403) is one no amount of waiting fixes,
and the agent does keep retrying: an instance stays blocked until you
renew the token in the Secret the StorageClass's `apiToken` points at.
The ref is re-resolved on every call, so the rotation takes effect on the
next reconcile without a restart.
