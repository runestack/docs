---
title: Init steps
lede: Run one-shot setup commands — like formatting a database's data volume — before a service's main process starts.
---
...
`runIf.path` is the path **as the container sees it**, not a path
relative to the volume root — so it has to sit under the `mountPath` of
a volume the step mounts. Rune translates it to the host side for you.
A path outside every mounted volume is rejected at `rune cast`:

```
initSteps[0] (seed-schema): runIf.path "/data/.seeded" is outside the mounted parent volume "data"
```
...
`securityContext.seccompProfile.type`. The validator rejects unknown
fields here at `rune cast` time with a "did you mean" hint.
:::

:::note[`type` values are case-insensitive]
The `type` values accept the Kubernetes PascalCase aliases
(`Unconfined`, `RuntimeDefault`, `Localhost`) alongside the lowercase
forms (`unconfined`, `default`, `localhost`). PascalCase is the
canonical form — it matches the Kubernetes docs.
:::
