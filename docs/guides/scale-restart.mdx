...
None of these are settings. Two coupled knobs — "how many extra" and "how far below" — can be set to a combination where an update can never take a step, and every value above has one right answer given what Rune already knows about the service. If you need to opt out of rolling entirely, that's `updateStrategy: recreate`.
