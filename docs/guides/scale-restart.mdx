...
So a `scale: 3` service with a readiness probe goes 3 → 4 → 3 → 4 → 3, never dropping below 3 serving instances, and finishes in a handful of reconcile passes. **Seeing four instances of a three-replica service mid-deploy is normal** — that's the spare, and it goes away when the update converges.

A service that can't run a spare goes 3 → 2 → 3 instead, one replica at a time. It's slower and dips by one, which is unavoidable when only one copy of a replica can exist.
...
The default never dips below `scale`, which means one replica at a time. For a `scale: 7` service that's seven sequential steps. If you're willing to run at 3/7 while deploying, say so:
...
Now Rune may retire four at a time — roughly two steps instead of seven. Omit it and nothing changes.
