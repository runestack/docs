...
rune release status my-app-prod         # owned resources, health, drift
...
Uninstall removes only what the release owns, in dependency order, honoring volume reclaim policy — shared `StorageClass` and `Namespace` resources are referenced, not owned, so they survive. By default a soft `uninstalled` tombstone is kept so the release can be reinstalled or rolled back; `--purge` forgets it.
